...
import com.Syufei.bybook.databinding.ActivityTryCarListBinding;
...
public class TryCarListActivity extends BaseActivity {
    ...
    private List<BookBean.TryBookList.Result> resultList;
    @Override
    ...
    private void queryData(){
        TryBookRepo.tryBookList(10, 1, new DataSuccessListenter() {
            @Override
            public void onDataSuccess(Object obj) {
                BookBean.TryBookList carList = (BookBean.TryBookList) obj;
    ...
    private void updateUI() {
        if (resultList==null || resultList.size()==0){
            ...
            for(BookBean.TryBookList.Result result:resultList){
                ...
                cancelBtn.setOnClickListener(view1 -> {
                    progressDialog.show();
                    BookBean.TryBookList.Result result1=(BookBean.TryBookList.Result) view1.getTag();
                    TryBookRepo.cancelTryBook(result1.getId(), new DataSuccessListenter() {
                        @Override
                        public void onDataSuccess(Object obj) {
                            UITools.showToast(TryCarListActivity.this,"取消成功");
                            int i=0;
                            for(;i<resultList.size();i++){
                                BookBean.TryBookList.Result current=resultList.get(i);
